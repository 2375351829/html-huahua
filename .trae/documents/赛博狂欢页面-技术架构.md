...
└── <script>            # 内联JavaScript代码
    ...
    ├── initUI()        # 初始化UI元素
    ├── initControls()  # 初始化控制面板
    ├── animate()       # 主动画循环
    ├── handleInteraction()  # 处理用户交互
    └── updateControls() # 更新控制面板状态
```
...
- 互动控制面板：使用HTML/CSS/JS实现可折叠的浮动控制面板，包含麦克风控制、音乐控制、视觉模式切换等功能
- 音频合成：使用Web Audio API创建立体声电子节拍循环，支持BPM调节和节奏预设
- 视觉模式：实现多种视觉风格切换，包括赛博狂欢、脉冲矩阵、全息蓝调、故障增强
- 色彩主题：通过修改CSS变量实现全局色彩主题切换
- 键盘快捷键：支持M（麦克风）、V（视觉模式）、D（数字雨）、Space（播放/暂停）、F（冻结）等快捷键