...
| 页面名称 | 模块名称 | 功能描述 |
|----------|----------|----------|
| 主页面 | 全屏加载与主视觉 | 3D粒子环背景、中央大标题故障效果、副标题打字机效果、底部频谱条 |
| 主页面 | 音频可视化核心区 | 合成器生成电子舞曲、环形频谱可视化、粒子随节拍脉冲、麦克风混合选项 |
| 主页面 | 数据流与全息卡片 | 三张磨砂玻璃卡片横向排列、卡片内小动画图标、鼠标视差效果 |
| 主页面 | 数字雨传送带 | Canvas数字雨效果、横向滚动标语、半透明叠加 |
| 主页面 | 结尾 | 3D粒子消散与聚合、故障文字闪烁、稳定后显示最终标语 |

## 3. 核心流程
用户打开页面 → 页面加载 → 自动播放音频和视觉效果 → 用户可滚动浏览不同区块 → 鼠标移动产生交互效果 → 音频可视化实时响应
...
| 页面名称 | 模块名称 | UI元素 |
|----------|----------|--------|
| 主页面 | 全屏加载与主视觉 | 3D粒子环背景、中央大标题（故障效果）、副标题（打字机效果）、扫描线动画、底部频谱条 |
| 主页面 | 音频可视化核心区 | 环形频谱（Canvas/Three.js）、粒子随节拍脉冲、实时频率数字显示、麦克风提示 |
| 主页面 | 数据流与全息卡片 | 三张磨砂玻璃卡片（backdrop-filter: blur(20px)）、霓虹边框、卡片内小动画图标、鼠标视差 |
| 主页面 | 数字雨传送带 | Canvas数字雨效果、半透明叠加、横向滚动标语 |
| 主页面 | 结尾 | 3D粒子消散与聚合、故障文字闪烁、最终标语显示 |
